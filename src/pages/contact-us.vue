<template>
  <div>
    ...
    <div v-if="viewMode === 'list'">
      ...
      <VCard
        v-for="contact in paginatedListContacts"
        :key="contact.id"
            ...
            <tr>
              ...
              <td width="15%">
                <VBtn
                  color="error"
                  variant="plain"
                  @click.stop="openDeleteDialog(contact)"
                >
    ...
    <div v-if="viewMode === 'grid'">
      <VRow>
        <VCol
          ...
          sm="4"
          md="4"
          v-for="contact in paginatedGridContacts.length
            ? paginatedGridContacts
            : contacts"
          :key="contact.id"
        >
...
const filterContacts = () => {
  ...
  filteredContacts.value = contacts.value.filter((contact: any) => {
    ...
    const matchesName =
      !filterName.value ||
      contact.name.toLowerCase().includes(filterName.value.toLowerCase());

    const matchesEmail =
      !filterEmail.value ||
      contact.email.toLowerCase().includes(filterEmail.value.toLowerCase());

    const matchesMessage =
      !filterMessage.value ||
      contact.message.toLowerCase().includes(filterMessage.value.toLowerCase());

    const matchesLocation =
      !filterLocation.value ||
      contact.location.toLowerCase().includes(filterLocation.value.toLowerCase());

    const matchesPhoneNumber =
      !filterPhoneNumber.value ||
      contact.phoneNumber.toLowerCase().includes(filterPhoneNumber.value.toLowerCase());
